...
class QBCloneAdvancedSetupTest(unittest.TestCase):
    ...
    def test_02_chart_of_accounts_template_selection(self):
        ...
        for template in templates:
            ...
            # Verify default accounts were created based on template
            # This assumes the backend creates default accounts based on the template
            # We would need to check if accounts were created in the database
            
            # For this test, we'll just verify the template was saved correctly
            response = requests.get(f"{BACKEND_URL}/company")
            self.assertEqual(response.status_code, 200)
            retrieved_company = response.json()
            self.assertEqual(retrieved_company["settings"]["chart_of_accounts_template"], template)
        
        logger.info("Chart of Accounts Template Selection tests passed")
    
    def test_03_data_types_and_optional_fields(self):
        ...
        # Retrieve the company to verify persistence of all data types
        response = requests.get(f"{BACKEND_URL}/company")
        self.assertEqual(response.status_code, 200)
        retrieved_company = response.json()
        
        # Verify all data types were retrieved correctly
        self.assertEqual(retrieved_company["settings"]["string_value"], "test string")
        self.assertEqual(retrieved_company["settings"]["integer_value"], 42)
        self.assertEqual(retrieved_company["settings"]["float_value"], 3.14159)
        self.assertEqual(retrieved_company["settings"]["boolean_value"], True)
        self.assertEqual(retrieved_company["settings"]["null_value"], None)
        self.assertEqual(retrieved_company["settings"]["array_value"], [1, 2, 3, 4, 5])
        self.assertEqual(retrieved_company["settings"]["object_value"]["nested_key"], "nested value")
        self.assertEqual(retrieved_company["settings"]["object_value"]["nested_array"], ["a", "b", "c"])
        self.assertEqual(retrieved_company["settings"]["date_value"], "2023-07-06T12:00:00Z")
