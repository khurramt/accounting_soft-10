        ...
        {activeTab === 'feeds' && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <h4 className="text-lg font-semibold text-gray-900">Bank Feeds</h4>
                ...
                <div key={feed.id} className="flex items-center justify-between p-3 border border-gray-200 rounded-lg">
                  <div className="flex items-center space-x-3">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                      feed.type === 'credit' ? 'bg-green-100' : 'bg-red-100'
                    }`}>
                      <span className="text-xs">
                        {feed.type === 'credit' ? '⬇️' : '⬆️'}
                      </span>
                    </div>
                    <div>
                      <div className="text-sm font-medium text-gray-900">{feed.description}</div>
                      <div className="text-xs text-gray-500">{feed.date}</div>
                    </div>
                  </div>
                  <div className="flex items-center space-x-3">
                    <div className={`text-sm font-medium ${
                      feed.type === 'credit' ? 'text-green-600' : 'text-red-600'
                    }`}>
                      {feed.type === 'credit' ? '+' : ''}${Math.abs(feed.amount).toLocaleString()}
                    </div>
